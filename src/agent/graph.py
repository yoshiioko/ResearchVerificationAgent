...
def build_graph(settings: Settings):
    ...
    # Wrap node calls so each node receives immutable settings explicitly.
    # The lambda is the adapter between LangGraph's call convention (state only)
    # and our node convention (state + settings).
    graph.add_node("researcher_node", lambda state: researcher_node(state, settings))
    graph.add_node("critic_node", lambda state: critic_node(state, settings))
    graph.add_node("compiler_node", lambda state: compiler_node(state, settings))
    ...
    graph.add_conditional_edges(
        "critic_node",
        lambda state: route_after_critic(state, settings),
        {
